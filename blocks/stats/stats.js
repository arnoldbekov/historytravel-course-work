import { appState } from '../../scripts/utils/data.js';
import { getPeriodName, getTypeName } from '../../scripts/utils/utils.js';
let periodModalOpenedAt = 0;
let periodModalOpening = false;
function buildStats() {
      ...
      options: {
        ...
        onClick: (evt, elements) => {
          if (elements.length > 0) {
            const now = Date.now();
            if (periodModalOpening || now - periodModalOpenedAt < 450) return;
            periodModalOpening = true;
            const periodKey = Object.keys(counts)[elements[0].index];
            openPeriodModal(periodKey);
            setTimeout(() => { periodModalOpening = false; }, 50);
          }
        },
...
function openPeriodModal(periodKey) {
  ...
  const isAlreadyOpen = modal.classList.contains('modal--visible') || modal.getAttribute('aria-hidden') === 'false';
  ...
  if (!isAlreadyOpen && typeof window.openModal === 'function') {
    window.openModal('periodModal');
  }
  periodModalOpenedAt = Date.now();
}
